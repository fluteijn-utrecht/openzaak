...
OPENZAAK_DOMAIN = config(
    ...
    "",
    help_text=(
        "a [host]:[port] or [host] value indicating the canonical domain where Open Zaak is hosted/deployed, e.g. openzaak.example.com:8443. This value is used (together with IS_HTTPS) when fully qualified URLs need to be constructed without HTTP request context available"
    ),
...
OPENZAAK_REWRITE_HOST = config(
    ...
    False,
    help_text=(
        "whether to rewrite the request host of all incoming requests with the value of OPENZAAK_DOMAIN, discarding the original Host header or headers set by reverse proxies. Useful if you provide the services only via the NLX network, for example. Defaults to False and conflicts with ``USE_X_FORWARDED_HOST``."
    ),
    auto_display_default=False,
)
...
MIN_UPLOAD_SIZE = config(
    "MIN_UPLOAD_SIZE",
    4 * 2**30,
    help_text="the max allowed size of POST bodies, in bytes. Defaults to 4GiB. Note that you should also configure your web server to allow this.",
)
...
DOCUMENTEN_UPLOAD_CHUNK_SIZE = config(
    ...
    MIN_UPLOAD_SIZE,
    help_text=(
        "chunk size in bytes for large file uploads - determines the size for a single upload chunk. Note that making this larger than ``MIN_UPLOAD_SIZE`` breaks large file uploads"
    ),
)
DOCUMENTEN_UPLOAD_READ_CHUNK = config(
    ...
    6 * 2**20,
    help_text=(
        "chunk size in bytes for large file uploads - when merging upload chunks, this determines the number of bytes read to copy to the destination file. Defaults to 6 MiB."
    ),
    auto_display_default=False,
)  # 6 MB default
...
SENDFILE_BACKEND = config(
    ...
    help_text=(
        "which backend to use for authorization-secured upload downloads. Defaults to sendfile.backends.nginx. See `django-sendfile2 <https://pypi.org/project/django-sendfile2/>`_ for available backends"
    ),
)
...
IMPORT_DOCUMENTEN_BATCH_SIZE = config(
    ...
    500,
    help_text=(
        "is the number of rows that will be processed at a time. Used for bulk importing ``EnkelvoudigInformatieObject``'s."
    ),
)
...
NOTIF_OPENZAAK_CONFIG_ENABLE = config(
    "NOTIF_OPENZAAK_CONFIG_ENABLE", default=True, add_to_docs=False
)
...
OPENZAAK_SELECTIELIJST_CONFIG_ENABLE = config(
    "OPENZAAK_SELECTIELIJST_CONFIG_ENABLE", default=True, add_to_docs=False
)
